...
void Setup() {
  ...
  snake.head->next =
      NodeNew(((gridWidth / 2) - 1) * scale, ((gridHeight / 2)) * scale);
  snake.length = 2;
...
void Update() {
  ...
  if (IsKeyDown(KEY_D) && snake.direction != Left)
    snake.nextDirection = Right;
  if (IsKeyDown(KEY_A) && snake.direction != Right)
    snake.nextDirection = Left;
  if (IsKeyDown(KEY_W) && snake.direction != Down)
    snake.nextDirection = Up;
  if (IsKeyDown(KEY_S) && snake.direction != Up)
  ...
  if (snake.head->x > (gridWidth - 1) * scale)
    snake.head->x = 0;
  if (snake.head->x < 0)
    snake.head->x = (gridWidth - 1) * scale;
  if (snake.head->y > (gridHeight - 1) * scale)
    snake.head->y = scale;
  if (snake.head->y < scale)
    snake.head->y = (gridHeight - 1) * scale;
...
void Draw() {
  if (!snake.isDead)
    DrawRectangle(apple.x, apple.y, scale, scale, WHITE);
  ...
  DrawText(TextFormat("Score: %i", (snake.length - 2) * scoreMultiplier),
           scale / 2, 1, scale, WHITE);

  if (snake.isDead) {
    DrawText(
        TextFormat("Final Score: %i", (snake.length - 2) * scoreMultiplier),
        scale / 2, scale * 2, scale * 2, WHITE);
  }

  if (showFPS)
    DrawFPS(scale / 2, screenHeight - scale);
}
